...
function applyDefaultAttrs(
  item: JSONContent,
  spec: Record<string, { default?: unknown }>,
) {
  const mergedAttrs = { ...getDefaultAttrs(spec), ...(item.attrs || {}) }
  item.attrs = Object.keys(mergedAttrs).length > 0 ? mergedAttrs : undefined
}
...
export function expandJSON(schema: Schema, json: JSONContent): JSONContent {
  function addDefaultsToNodeOrMark(
    ...
    if (itemType?.spec.attrs) {
      applyDefaultAttrs(item, itemType.spec.attrs)
    }
...
const activeClass = (isActive?: boolean) => (isActive ? ACTIVE_CLASS : "")
    ...
    <div className="container inset-shadow-2xs mx-auto flex h-full min-h-200 w-full flex-col overflow-hidden rounded-md border font-sans">
      <div className="flex items-center gap-x-2 border-b bg-card p-2 text-muted-foreground">
        ...
        <ButtonGroup>
          ...
          <Button
            className={activeClass(editorState?.isBulletList)}
          ...
          <Button
            className={activeClass(editorState?.isOrderedList)}
        ...
        <ButtonGroup>
          <Button
            className={activeClass(editorState?.isBold)}
          ...
          <Button
            className={activeClass(editorState?.isItalic)}
          ...
          <Button
            className={activeClass(editorState?.isStrike)}
          ...
          <Button
            className={activeClass(editorState?.isCode)}
          ...
          <Button
            className={activeClass(editorState?.isUnderline)}
        ...
        <ButtonGroup>
          ...
          >
            <DropdownMenuTrigger
              render={
                <Button
                  className={activeClass(editorState?.isLink)}
                  size="icon-sm"
                  variant="outline"
                >
                  <Link />
          ...
          <Button
            className={activeClass(editorState?.isSuperscript)}
          ...
          <Button
            className={activeClass(editorState?.isSubscript)}
        ...
        <ButtonGroup>
          <Button
            className={activeClass(editorState?.isAlignLeft)}
          ...
          <Button
            className={activeClass(editorState?.isAlignCenter)}
          ...
          <Button
            className={activeClass(editorState?.isAlignRight)}
          ...
          <Button
            className={activeClass(editorState?.isAlignJustify)}
            onClick={() =>
        ...
        <ButtonGroup>
          <Button
            className={activeClass(editorState?.isCodeBlock)}
          ...
          <Button
            className={`font-serif text-5xl ${activeClass(editorState?.isBlockquote)}`}
